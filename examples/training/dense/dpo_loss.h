// moe/dpo_loss.h - DPO loss computation for Dense models
#pragma once
...
inline float compute_ref_logp(
    ...
    // ggml_cross_entropy_loss: 수치적으로 안정, 토큰당 평균 CE 반환
...
inline dpo_step_result dpo_training_step(
    ...
    struct ggml_tensor * avg_ce_c = ggml_cross_entropy_loss(ctx, logits_c, t_targets_c);
    struct ggml_tensor * avg_ce_r = ggml_cross_entropy_loss(ctx, logits_r, t_targets_r);
    struct ggml_tensor * log_p_c = ggml_neg(ctx, avg_ce_c);
    struct ggml_tensor * log_p_r = ggml_neg(ctx, avg_ce_r);
