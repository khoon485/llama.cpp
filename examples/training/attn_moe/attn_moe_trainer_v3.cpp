...
// ============================================================================
// End-to-end forward pass: all 24 layers + final projection
// ============================================================================

static end_to_end_output build_end_to_end_forward(
    struct ggml_context * ctx,
    const all_layer_hidden_states & frozen_states,
    std::vector<struct ggml_tensor *> & router_w,
    std::vector<struct ggml_tensor *> & o_router_w,
    std::vector<struct ggml_tensor *> & q_lora_a,
    std::vector<struct ggml_tensor *> & q_lora_b,
    std::vector<struct ggml_tensor *> & k_lora_a,
    std::vector<struct ggml_tensor *> & k_lora_b,
    std::vector<struct ggml_tensor *> & v_lora_a,
    std::vector<struct ggml_tensor *> & v_lora_b,
    std::vector<struct ggml_tensor *> & o_lora_a,
    std::vector<struct ggml_tensor *> & o_lora_b,
    std::vector<struct ggml_tensor *> & layer_frozen,
    struct ggml_tensor * lm_head,
    struct ggml_tensor * output_norm,
    float lora_scale,
    int n_layers
) {
    int n_embd = frozen_states.n_embd;
    int n_tokens = frozen_states.n_tokens;
    ...
    // Project to vocab
    struct ggml_tensor * logits = ggml_mul_mat(ctx, lm_head, normalized);

    end_to_end_output result;
